# Near qr code data transmission protocol
...
For sending the unsigned data from a watch-only wallet to an offline signer. a new bc-ur type `near-sign-request` will be introduced for encoding the signing request.
...
After signing the data offline signer should send the signature back to the watch-only wallet. and a new bc-ur type called `near-signature` introduced here to encode the data.
